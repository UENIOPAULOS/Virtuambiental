...
DB_PATH = os.environ.get('DATABASE_URL_SQLITE', os.path.join('/data' if os.path.isdir('/data') else '/tmp', 'app.db'))
...
class Company(db.Model):
    ...
    tax_id = db.Column(db.String(50), unique=True, nullable=True)
...
class License(db.Model):
    ...
    license_type = db.Column(db.String(200), nullable=False)
...
def calc_stats(items):
    ...
    total = len(items); ok_30 = 0; ok_60 = 0
    for lic in items:
        ...
        if lic.expiry_date:
            ...
            if lic.expiry_date > today + timedelta(days=30): ok_30 += 1
            if lic.expiry_date > today + timedelta(days=60): ok_60 += 1
...
def company_stats(company_id: int):
    return calc_stats(License.query.filter_by(company_id=company_id).all())

def global_stats():
    return calc_stats(License.query.all())

def get_settings():
    return AlertSettings.query.first()

def send_email(settings, subject, body):
    recips = [r.strip() for r in (settings.recipients or '').split(',') if r.strip()]
    if not recips: return False, "Sem destinatários"
    ...
    try:
        if settings.security == 'ssl':
            ...
            if settings.security == 'starttls': srv.starttls()
        if settings.smtp_user: srv.login(settings.smtp_user, settings.smtp_pass or '')
        srv.sendmail(settings.from_email, recips, msg.as_string())
        srv.quit()
        return True, None
...
def format_digest(items_by_threshold):
    lines = ["Resumo de licenças com vencimento próximo:\n"]
...
def run_alerts():
    s = get_settings()
    if not s: return False, "Configurações de alerta não encontradas."
    try:
    ...
    for t in thresholds:
        ...
        for lic in License.query.filter(License.expiry_date >= today, License.expiry_date <= limit).all():
            from sqlalchemy import exists, and_
            already = Notification.query.filter_by(license_id=lic.id, threshold=t).first()
            if not already: items_by_threshold[t].append(lic)
    if not any(items_by_threshold.values()): return True, "Nenhum alerta a enviar."
    ok, err = send_email(s, "Alertas de Vencimento de Licenças", format_digest(items_by_threshold))
...
def dashboard():
    ...
    return render_template('dashboard.html', total_companies=total_companies, total_licenses=total_licenses, due_30=due_30, due_60=due_60, expired=expired, upcoming=upcoming, today=today)
...
def companies_new():
    if request.method == 'POST':
        c = Company(name=request.form['name'], tax_id=request.form.get('tax_id') or None, sector=request.form.get('sector'), state=request.form.get('state'), city=request.form.get('city'), contact_email=request.form.get('contact_email'), contact_phone=request.form.get('contact_phone'))
...
@app.route('/licenses')
def licenses():
    status = request.args.get('status', ''); horizon = request.args.get('horizon', '30'); q = request.args.get('q', '').strip()
    from sqlalchemy import or_
    qry = License.query.join(Company)
    if status: qry = qry.filter(License.status == status)
    if q:
        like = f"%{q}%"
...
def licenses_new():
    ...
    if request.method == 'POST':
        lic = License(company_id=int(request.form['company_id']), authority=request.form['authority'], license_type=request.form['license_type'], number=request.form.get('number') or None, issue_date=parse_date(request.form.get('issue_date')), expiry_date=parse_date(request.form.get('expiry_date')), status=request.form.get('status') or 'Ativa', notes=request.form.get('notes') or None)
...
def api_company_stats(cid):
    ...
    return global_stats() if cid == -1 else company_stats(cid)
...
def alerts_settings():
    ...
    if request.method == 'POST':
        ...
        if request.form.get('smtp_pass'): s.smtp_pass = request.form.get('smtp_pass')
...
def alerts_run():
    ok, msg = run_alerts()
    flash((msg or 'Alertas enviados.') if ok else f'Falha ao enviar: {msg}', 'success' if ok else 'danger')
    return redirect(url_for('alerts_settings'))
...
def alerts_send_test():
    ...
    flash('E-mail de teste enviado.' if ok else f'Falha ao enviar teste: {err}', 'success' if ok else 'danger')
    return redirect(url_for('alerts_settings'))
